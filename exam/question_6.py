...
def longest_words(filename):
    ...
    for sent in sentences:
        sentences_final.append(sent.strip().split())
    sentences_final = [[w.replace(':', '') for w in sent] for sent in sentences_final]
    sentences_final = [[w.replace(',', '') for w in sent] for sent in sentences_final]
    sentences_final = [[w.replace(';', '') for w in sent] for sent in sentences_final]
    max_len = -1
    for sent in sentences_final:
        for word in sent:
            if len(word.strip()) > max_len:
                max_len = len(word.strip())

    longest_words = defaultdict(list)

    for i, sent in enumerate(sentences_final):
        for j, word in enumerate(sent):
            if len(word.strip()) == max_len:
                longest_words[word.strip().lower().title()].append((i + 1, j + 1))
